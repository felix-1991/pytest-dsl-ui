...
class BrowserHTTPRequest:
    ...
    def process_asserts(self, specific_asserts=None, index_mapping=None) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        for assertion_idx, assertion in enumerate(process_asserts):
            ...
            # 解析断言参数（复用pytest-dsl的解析逻辑）
            extraction_path, assertion_type, compare_operator, expected_value = self._parse_assertion_params(assertion)
                ...
                if len(failed_assertions) == 1:
                    ...
                    for idx, (assertion_idx, error_msg) in enumerate(failed_assertions, 1):
                        if "[" in error_msg and "]" in error_msg:
                            extractor_type = error_msg.split("[", 1)[1].split("]")[0]
                        else:
                            extractor_type = "未知"
    ...
    def _parse_assertion_params(self, assertion: List[Any]) -> Tuple[str, str, str, Any]:
        """解析断言参数（复用pytest-dsl逻辑）"""
        # 这里实现与pytest-dsl相同的断言参数解析逻辑
        # 为了简洁，这里只实现核心逻辑
        extractor_type = assertion[0]
        ...
        if len(assertion) == 2:
            ...
            extraction_path = assertion[1]
            if assertion[2] in COMPARISON_OPERATORS:
                assertion_type = "value"
                compare_operator = assertion[2]
        ...
        return extraction_path, assertion_type, compare_operator, expected_value
                ...
                if isinstance(expected_value, str):
                    ...
                    if clean_expected.isdigit() or (clean_expected.startswith('-') and clean_expected[1:].isdigit()):
                        expected_value = int(clean_expected)
                         ...
                         (clean_expected.startswith('-') and clean_expected[1:].replace('.', '', 1).isdigit()):
                ...
                if isinstance(actual_value, str):
                    ...
                    if clean_actual.isdigit() or (clean_actual.startswith('-') and clean_actual[1:].isdigit()):
                        actual_value = int(clean_actual)
                         ...
                         (clean_actual.startswith('-') and clean_actual[1:].replace('.', '', 1).isdigit()):
