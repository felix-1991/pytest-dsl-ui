...
@keyword_manager.register('启动浏览器', [
    {'name': '浏览器', 'mapping': 'browser_type', 'description': '浏览器类型：chromium, firefox, webkit'},
    {'name': '无头模式', 'mapping': 'headless', 'description': '是否以无头模式运行，默认为true'},
    {'name': '慢动作', 'mapping': 'slow_mo', 'description': '操作间隔时间（毫秒），用于调试'},
        ...
        try:
            ...
            if isinstance(config_str, str):
                try:
                    ...
                    # 尝试作为YAML解析
                    import yaml
                    config = yaml.safe_load(config_str) if config_str.strip() else {}
            else:
            ...
            # 启动浏览器
            browser_id = browser_manager.launch_browser(browser_type, **launch_config)
            
            # 创建默认上下文
            context_config = {}
            if width and height:
                context_config['viewport'] = {'width': int(width), 'height': int(height)}
...
@keyword_manager.register('关闭浏览器', [
    {'name': '浏览器ID', 'mapping': 'browser_id', 'description': '要关闭的浏览器ID，如果不指定则关闭当前浏览器'},
])
...
@keyword_manager.register('新建页面', [
    {'name': '上下文ID', 'mapping': 'context_id', 'description': '浏览器上下文ID，如果不指定则使用当前上下文'},
])
def new_page(**kwargs):
    """新建页面
