...
class BrowserHTTPClient:
    ...
    def make_request(self, method: str, url: str, **request_kwargs) -> 'BrowserResponse':
        ...
        try:
            # 发送请求
            if method.upper() == 'GET':
                response = self._api_request_context.get(url, **playwright_kwargs)
            elif method.upper() == 'POST':
                response = self._api_request_context.post(url, **playwright_kwargs)
            elif method.upper() == 'PUT':
                response = self._api_request_context.put(url, **playwright_kwargs)
            elif method.upper() == 'DELETE':
                response = self._api_request_context.delete(url, **playwright_kwargs)
            elif method.upper() == 'PATCH':
                response = self._api_request_context.patch(url, **playwright_kwargs)
            elif method.upper() == 'HEAD':
                response = self._api_request_context.head(url, **playwright_kwargs)
            else:
